...
function Shell({title,subtitle,icon:Icon,children}:{title:string;subtitle:string;icon:any;children:React.ReactNode}){
  return <main className="min-h-screen md:pl-[252px]">
    <div className="mx-auto max-w-[1500px] px-5 pb-12 pt-20 md:px-8 md:pt-8 xl:px-10">
      <header className="animate-in relative mb-7 overflow-hidden rounded-[24px] border border-white/[.06] bg-gradient-to-br from-white/[.03] via-white/[.015] to-transparent px-5 py-5 md:px-6">
        <div className="pointer-events-none absolute -right-16 -top-24 h-56 w-56 rounded-full bg-violet-500/[.10] blur-3xl"/>
        <div className="relative flex items-center gap-4">
          <div className="icon-shell h-11 w-11 text-violet-300"><Icon size={18}/></div>
          <div>
            <p className="section-kicker">LifeOS module</p>
...
function WorkView({
  ...
  return <Shell title="Work" subtitle="Log daily work, keep long-running tasks open, and review weekly or monthly output." icon={Briefcase}>
    ...
    <section className="grid gap-4 xl:grid-cols-[1.4fr_1fr]">
      <form onSubmit={submitTask} className="card p-5 md:p-6">
        <div className="mb-5 flex items-start justify-between gap-4"><div><p className="section-kicker">Quick capture</p><h2 className="panel-heading">Log a work task</h2><p className="panel-subtitle">Record completed work or keep a multi-day task in progress.</p></div><span className="icon-shell text-violet-300"><Plus size={16}/></span></div>
        ...
        <button disabled={savingTask} className="btn-primary mt-3 w-full">{savingTask?'Saving...':'Save task'}</button>
      </form>

      <form onSubmit={submitProject} className="card p-5 md:p-6">
        <div className="mb-5 flex items-start justify-between gap-4"><div><p className="section-kicker">Structure</p><h2 className="panel-heading">New project</h2><p className="panel-subtitle">Group related tasks into a focused workstream.</p></div><span className="icon-shell text-cyan-300"><FolderKanban size={16}/></span></div>
        <div className="space-y-3">
          ...
          <button disabled={savingProject} className="btn-secondary w-full">{savingProject?'Saving...':'Create project'}</button>
        </div>
      </form>
    </section>
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.6fr_1fr]">
      <div className="card p-5 md:p-6">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div><p className="section-kicker">Output</p><h2 className="panel-heading">Completed tasks</h2><p className="panel-subtitle">Daily completion trend</p></div>
      ...
      <div className="card p-5 md:p-6">
        ...
        <h2 className="panel-heading">Category breakdown</h2>
        <p className="panel-subtitle">Completed tasks by work type</p>
    ...
    <section className="mt-4 card p-5 md:p-6">
      <div className="flex items-start gap-3"><span className="icon-shell h-9 w-9 text-cyan-300"><Sparkles size={15}/></span><div><p className="section-kicker">Insight</p><h2 className="panel-heading">Automatic summary</h2><p className="mt-3 text-sm leading-7 text-[#b9bdc8]">{report?.work.summary||'No report data yet.'}</p></div></div>
    </section>

    <section className="mt-4 card overflow-hidden">
      <div className="flex items-center justify-between border-b border-white/[.06] p-5">
        <div><p className="section-kicker">Archive</p><h2 className="panel-heading">Completed work</h2><p className="panel-subtitle">{completedTasks.length} finished tasks</p></div>
    ...
    <section className="mt-4 card overflow-hidden">
      <div className="border-b border-white/[.055] p-5 md:px-6"><p className="section-kicker">Task stream</p><h2 className="panel-heading">All tasks</h2><p className="panel-subtitle">{tasks.length} recorded tasks</p></div>
...
function LearningView({
  ...
  return <Shell title="Learning" subtitle="Create a course or book goal, log today's study session, and watch the trend build automatically." icon={BookOpen}>
    ...
    <section className="grid gap-4 xl:grid-cols-2">
      <form onSubmit={submitGoal} className="card p-5 md:p-6">
        ...
        <div className="space-y-3">
          ...
          <button disabled={savingGoal} className="btn-primary w-full">{savingGoal?'Saving...':'Start goal'}</button>
        </div>
      </form>

      <form onSubmit={submitSession} className="card p-5 md:p-6">
        <div className="mb-5"><p className="section-kicker">Daily session</p><h2 className="panel-heading">Log today's study</h2><p className="panel-subtitle">One quick session at a time</p></div>
        <div className="space-y-3">
          ...
          <button disabled={savingSession||!selected} className="btn-secondary w-full border-violet-400/15 bg-violet-500/[.07] text-violet-100">{savingSession?'Saving...':'Log session'}</button>
        </div>
      </form>
    </section>
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-2">
      <div className="card p-5 md:p-6">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div><p className="section-kicker">Focus time</p><h2 className="panel-heading">Study hours trend</h2><p className="panel-subtitle">Hours studied per day</p></div>
          <PeriodSwitch value={reportPeriod} onChange={onReportPeriodChange}/>
        </div>
      ...
      <div className="card p-5 md:p-6">
        <div className="mb-5"><p className="section-kicker">Reading</p><h2 className="panel-heading">Pages read trend</h2><p className="panel-subtitle">Book pages logged per day</p></div>
        <div className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
    ...
    <section className="mt-4 card p-5 md:p-6">
      <div className="flex items-start gap-3"><span className="icon-shell h-9 w-9 text-orange-300"><Flame size={16}/></span><div><p className="section-kicker">Momentum</p><h2 className="panel-heading">Automatic learning summary</h2><p className="mt-2 text-sm leading-7 text-[#b9bdc8]">{report?.learning.summary||'No learning activity logged yet.'}</p></div></div>
    </section>
    ...
    <section className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {learning.length?learning.map(item=>{
        ...
        return <article key={item._id} className="card p-5 md:p-6">
      ...
      }):<div className="card p-5 md:p-6"><Empty text="No learning goals yet."/></div>}
    </section>
  </Shell>;
}
  ...
  return <Shell title="Daily review" subtitle="Capture what you learned, blockers, and tomorrow's focus." icon={CalendarDays}>
    ...
    <section className="grid gap-4 xl:grid-cols-[1fr_1.2fr]">
      <form onSubmit={submit} className="card p-5 md:p-6">
        <p className="section-kicker">Reflection</p><h2 className="panel-heading">New review</h2><p className="panel-subtitle">Close the day with a short structured reflection.</p>
        <div className="mt-4 space-y-3">
          ...
          <button disabled={saving} className="btn-primary w-full">{saving?'Saving...':'Save daily review'}</button>
        </div>
      </form>

      <div className="card overflow-hidden">
        <div className="border-b border-white/[.055] p-5 md:px-6"><p className="section-kicker">Timeline</p><h2 className="panel-heading">Review history</h2><p className="panel-subtitle">{reviews.length} entries</p></div>
...
function SettingsView({username,onLogout}:{username:string;onLogout:()=>void}){
  return <Shell title="Settings" subtitle="Current local workspace configuration." icon={SettingsIcon}>
    <section className="card max-w-3xl p-5 md:p-6">
      ...
      <button onClick={onLogout} className="btn-secondary border-rose-400/15 bg-rose-500/[.06] text-rose-200"><LogOut size={16}/>Log out</button>
    </section>
  </Shell>;
}

function PeriodSwitch({value,onChange}:{value:ReportPeriod;onChange:(period:ReportPeriod)=>Promise<void>}){
  return <div className="flex rounded-[12px] border border-white/[.06] bg-black/20 p-1 shadow-inner">
    <button onClick={()=>void onChange('week')} className={`rounded-[9px] px-3 py-2 text-[11px] font-medium transition ${value==='week'?'bg-white/[.09] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.06)]':'text-[#737785] hover:text-white'}`}>Week</button>
    <button onClick={()=>void onChange('month')} className={`rounded-[9px] px-3 py-2 text-[11px] font-medium transition ${value==='month'?'bg-white/[.09] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.06)]':'text-[#737785] hover:text-white'}`}>Month</button>
  </div>;
}
